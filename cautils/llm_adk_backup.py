...
weather_sentiment_agent = Agent(
    ...
    tools=[FunctionTool(func=metadata_tool.get_table_info)],
)
...
async def main():
    ...
    # query = "get fields from table broadcast_tv_radio_station in dataset fcc_political_ads in project as-alf-argolis
    query = """get fields from table broadcast_tv_radio_station in dataset fcc_political_ads in project as-alf-argolis
    list all fields, and all data you can from each field.
    list subfields of records, use the "subfields" key for those.
    infer descriptions for fields and add them to the output.
